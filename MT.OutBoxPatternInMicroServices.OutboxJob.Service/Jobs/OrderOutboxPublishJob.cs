...
using MT.OutBoxPatternInMicroServices.OutboxJob.Service.Context;
...
namespace MT.OutBoxPatternInMicroServices.OutboxJob.Service.Jobs
{
    public class OrderOutboxPublishJob : IJob
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            if (OrderSingletonDatabase.DataReaderState)
            {
                ...
                List<Domain.Entities.OrderOutBox> orderOutboxes = (await OrderSingletonDatabase.QueryAsync< Domain.Entities.OrderOutBox> ($@"SELECT * FROM OrderOutboxes
                                                                                                           WHERE ProcessedDate IS NULL
                                                                                                           ORDER By OccuredOn DESC"))
                                                                        .ToList();
                foreach (Domain.Entities.OrderOutBox orderOutbox in orderOutboxes)
                {
